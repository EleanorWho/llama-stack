...
class TestLiteLLMOpenAIMixinSafetyIdentifierPassing:
    """Test cases for safety_identifier parameter passing through LiteLLM"""
    ...
    async def test_chat_completion_passes_safety_identifier(self, mixin_with_model_store):
        """Test that safety_identifier is passed to LiteLLM chat completions"""
        with patch("litellm.acompletion", new_callable=AsyncMock) as mock_acompletion:
            mock_acompletion.return_value = MagicMock()
            ...
            await mixin_with_model_store.openai_chat_completion(
                OpenAIChatCompletionRequestWithExtraBody(
                    model="test-model",
                    messages=[OpenAIUserMessageParam(role="user", content="Hello")],
                    safety_identifier="user-456-hashed",
                )
            )
